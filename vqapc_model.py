...
class GumbelAPCModel(nn.Module):
    def __init__(self, input_size, hidden_size, num_layers, dropout, residual,
        ...
        ## Original code use GRU,here we use LSTM
        # assert num_layers > 0
        ...
        self.rnn_layers = nn.LSTM(input_size=input_size,
                            hidden_size=hidden_size,
                            num_layers=num_layers,
                            batch_first=True)


        # Not use dropout nor residual
    ...
    def forward(self, frames_BxLxM, seq_lengths_B, testing):
        ...
        #     packed_rnn_outputs, _ = rnn_layer(frames_BxLxM)

            # Unpack RNN output of current layer.
            # rnn_outputs_BxLxH, _ = pad_packed_sequence(packed_rnn_outputs,
            #                                         batch_first=True,
            #                                         total_length=max_seq_len)
        packed_rnn_outputs, _ = self.rnn_layers(frames_BxLxM)

        # Apply dropout to output.
        rnn_outputs_BxLxH = self.rnn_dropout(packed_rnn_outputs)
        ...
        if self.vq_layers[0] is not None:
            logits_BxLxC, rnn_outputs_BxLxH = self.vq_layers[0](rnn_outputs_BxLxH, testing)
...
if __name__ == '__main__':
    model = GumbelAPCModel(input_size=40,
                         hidden_size=512,
                         num_layers=4,
                         dropout=0,
                         residual=False,
